...
int x,y;
void setup() {
  myServoX.attach(2); // ขาที่ D4
  myServoY.attach(0); // ขาที่ D3
  myServoZ.attach(5); // ขาที่ D1
  Serial.begin(9600);
  myServoX.write(90);
  myServoY.write(90);
  myServoZ.write(0);
...
void loop() {
    ...
    client.setTimeout(0);
     ...
     while (client.connected()) {
      if (client.available() > 0) {
        x = client.parseInt(); // รับค่า x จาก Python script
  ...
  {
    if (client.available() > 0) {
    y = client.parseInt(); // รับค่า y จาก Python script
